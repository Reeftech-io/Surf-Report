...
async function queueLPWithdrawAll() {
    try {
        const address = globalAddress;
        const errorElement = document.getElementById('address-error-lp');
        const lpAsset1Display = document.getElementById('lp-asset1-display');
        const lpAsset2Display = document.getElementById('lp-asset2-display');

        if (!lpAsset1Display || !lpAsset2Display || !errorElement) {
            errorElement.textContent = 'Liquidity pool elements not found.';
            log('Error: Liquidity pool elements not found.');
            return;
        }

        const asset1 = lpAsset1Display.getAttribute('data-value') || lpAsset1Display.textContent;
        const asset2 = lpAsset2Display.getAttribute('data-value') || lpAsset2Display.textContent;

        if (!asset1 || !asset2 || asset1 === 'Select Asset' || asset2 === 'Select Asset') {
            errorElement.textContent = 'Please select both pool assets.';
            log('Error: Please select both pool assets.');
            return;
        }
        ...
        if (!isPoolDataLoaded) {
            log('Pool data not loaded, refreshing...');
            await checkLPPool();
        }

        if (!isPoolDataLoaded || !lpTokenCurrency || !lpTokenIssuer) {
            errorElement.textContent = 'Pool data not loaded. Please check pool first.';
            log('Error: Pool data not loaded. Please check pool first.');
            return;
        }
        ...
        const asset2Data = asset2 === "XRP" ? { currency: "XRP" } : { currency: getAssetByName(asset2).hex, issuer: getAssetByName(asset2).issuer };
        if (!asset1Data || !asset2Data) {
            errorElement.textContent = `Invalid asset pair: ${asset1}/${asset2}.`;
            log(`Error: Invalid asset pair: asset1=${JSON.stringify(asset1Data)}, asset2=${JSON.stringify(asset2Data)}`);
            return;
        }


        log(`Asset Definitions: asset1=${JSON.stringify(asset1Data)}, asset2=${JSON.stringify(asset2Data)}`);
        ...
        const trustline = accountLines.result.lines.find(line => 
            ...
            line.account === lpTokenIssuer && 
            parseFloat(line.balance) > 0
        );
        if (!trustline) {
            errorElement.textContent = `No LP tokens found for ${asset1}/${asset2}.`;
            log(`Error: No LP tokens found for ${asset1}/${asset2}.`);
            return;
        }

        
        if (trustline.currency !== lpTokenCurrency || trustline.account !== lpTokenIssuer) {
            errorElement.textContent = `LP token mismatch for ${asset1}/${asset2}.`;
            log(`Error: LP token mismatch: trustline.currency=${trustline.currency}, trustline.issuer=${trustline.account}, expected.currency=${lpTokenCurrency}, expected.issuer=${lpTokenIssuer}`);
            return;
        }
        ...
        const preparedTx = await client.autofill(withdrawTx);
        preparedTx.LastLedgerSequence = ledgerInfo.result.ledger_current_index + 100;
        log(`Ledger Current Index: ${ledgerInfo.result.ledger_current_index}, LastLedgerSequence: ${preparedTx.LastLedgerSequence}`);
        ...
        if (txResponse.result.meta.TransactionResult !== 'tesSUCCESS') {
            errorElement.textContent = `Transaction failed: ${txResponse.result.meta.TransactionResult}`;
            log(`Error: Transaction failed: ${txResponse.result.meta.TransactionResult}`);
            return;
        }
        ...
        if (txResponse.result.meta.AffectedNodes) {
            for (const node of txResponse.result.meta.AffectedNodes) {
                if (node.ModifiedNode && node.ModifiedNode.LedgerEntryType === "AccountRoot" && node.ModifiedNode.FinalFields.Account === address) {
                ...
                if (node.ModifiedNode && node.ModifiedNode.LedgerEntryType === "RippleState" && node.ModifiedNode.FinalFields.LowLimit.issuer === address) {
                    const previousBalance = parseFloat(node.ModifiedNode.PreviousFields.Balance || "0");
                    const finalBalance = parseFloat(node.ModifiedNode.FinalFields.Balance || "0");
        ...
        const hash = txResponse.result.hash;
        const explorerLink = `https://xrpscan.com/tx/${hash}`;
        log(`Successfully withdrew ${formatBalance(trustline.balance)} LP tokens from ${asset1}/${asset2} pool. Hash: ${hash}`);
        log(`Withdrawn: ${formatBalance(withdrawnXrp)} ${asset1IsXRP ? asset1 : asset2}, ${formatBalance(withdrawnToken)} ${tokenAsset}`);
        log(`Confirmation: ${hash}`, true);

        const lpQueue = document.getElementById('lp-queue');
        if (lpQueue) {
            lpQueue.innerHTML = `
                <p>Transaction Queue:</p>
                <p>Withdrawal Result: Withdrew ${formatBalance(withdrawnXrp)} ${asset1IsXRP ? asset1 : asset2} and ${formatBalance(withdrawnToken)} ${tokenAsset}${withdrawnToken === 0 ? ' (Note: 0 token amount due to pool imbalance or rounding)' : ''}. Transaction: <a href="${explorerLink}" target="_blank">${hash}</a></p>
            `;
        }

        errorElement.innerHTML = `Successfully withdrew all LP tokens from ${asset1}/${asset2}. Transaction: <a href="${explorerLink}" target="_blank">${hash}</a>`;
        ...
        log(`Error queuing withdraw all: ${error.message}`);
    }
}
