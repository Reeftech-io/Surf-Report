document.addEventListener("DOMContentLoaded", () => {
    ...
    const initializeConfirmedAssets = async () => {
        for (const asset of assets) {
            ...
            if (await checkImageExists(src)) {
                confirmedAssets.push({ ...asset, src });
            }
        }
        if (confirmedAssets.length === 0) {
            confirmedAssets.push({ name: "XRP", issuer: "fallback", src: "icons/XRP.png" });
        }
    };

    const getRandomImageSrc = () => {
        const randomIndex = Math.floor(Math.random() * confirmedAssets.length);
        return confirmedAssets[randomIndex].src;
    };

    const createAnimatedIcon = async () => {
        if (confirmedAssets.length === 0) {
            await initializeConfirmedAssets();
        }
        const icon = document.createElement("img");
        icon.classList.add("animated-icon");
        icon.src = getRandomImageSrc();

        const angle = Math.random() * 360;
        const radians = (angle * Math.PI) / 180;
        const speed = Math.random() * 10 + 5;
        const distance = Math.sqrt(window.innerWidth ** 2 + window.innerHeight ** 2) + 100;
        ...
        const keyframeName = `move${Date.now()}`;
        ...
        icon.style.animation = `${keyframeName} ${speed}s linear forwards`;

        const shouldRotate = Math.random() > 0.5;
        if (shouldRotate) {
            const rotationSpeed = Math.random() * 2 + 1;
            icon.style.animation += `, rotate ${rotationSpeed}s linear infinite`;
        }
        ...
        const checkOverlap = () => {
            ...
            const uiElements = document.querySelectorAll(".container, .sidebar-left-container, .sidebar-right-container");
            ...
            icon.style.opacity = isOverlapping ? "0.01" : "1";
        };
    ...
    for (let i = 0; i < numberOfIcons; i++) {
        setTimeout(createAnimatedIcon, i * 1000);
    }
});